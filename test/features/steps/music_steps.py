...
# Note: We reuse the steps from history_steps.py for the "When I type" and "Then the output should contain"
# Those are more generic, but they need the "REPL is running" context set up correctly here.
...
def step_impl_repl_running(context):
    if not hasattr(context, "repl"):
        ...
        import sys
        import lib.core.music
        
        # Patch init and methods that throw uninitialized errors
        lib.core.music.pygame.mixer.init = MagicMock()
        lib.core.music.pygame.mixer.music.load = MagicMock()
        lib.core.music.pygame.mixer.music.play = MagicMock()
        lib.core.music.pygame.mixer.music.stop = MagicMock()
        lib.core.music.pygame.mixer.music.pause = MagicMock()
        lib.core.music.pygame.mixer.music.unpause = MagicMock()
        lib.core.music.pygame.mixer.music.set_volume = MagicMock()
        
        state = StateFactory.create(context.gamedir)
        context.state = state
        
        # Stop the daemon thread from MusicManager for tests cleanly so it doesn't hang
        state.music_manager.is_playing = False
