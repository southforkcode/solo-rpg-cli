...
class SummaryPrinter(PrettyPrinter):
    ...
    def print(self, obj: GameSummary) -> None:
        ...
        if obj.journeys:
            journey_table = Table(show_header=True, header_style="bold magenta", expand=True)
            ...
            journeys_panel = Panel(journey_table, title="[bold magenta]Active Journeys[/bold magenta]", border_style="magenta", padding=(1, 1))
        else:
            journeys_panel = Panel("[italic dim]No active journeys at the moment.[/italic dim]", title="[bold magenta]Active Journeys[/bold magenta]", border_style="magenta")
        ...
        if obj.journals:
            ...
            journal_panel = Panel(entries_text, title="[bold green]Recent Journal Entries[/bold green]", border_style="green", padding=(1, 2))
        else:
            journal_panel = Panel("[italic dim]The journal is currently empty.[/italic dim]", title="[bold green]Recent Journal Entries[/bold green]", border_style="green")

        console.print()
        console.print(Panel.fit(
            "[bold cyan]Solo RPG Game Summary[/bold cyan]",
            border_style="cyan"
        ))
